...
В пользователи `app` нужно хранить ключи которые редко изменяться, и часто используются, или если этот ключ больше некуда(нелогично) сохранить.  
...
4. `_path` - Хранения основных путей программы.
   ...
   - `vnev_python` - Относительный путь(по отношению к `app`) к интерпретатору в виртуальном окружение
