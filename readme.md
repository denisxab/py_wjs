...
В обычном понимание, установка программа на `PywJs` не нужна. Что `html` файл, сразу готов к запуску, что `python`(при наличии зависимых модулей) сразу готов запуску. Но все же, для пользователей которые, не знаю кто такой `pip`, и как открывать консоль, нуждаются в некоторой автоматизации.
...
- `ИмяПрограммы`
  - `client` - Для фронта
    - `index.html`
  - `server` - Для бека
    - `main.py` - Указываем [WbsHandle](#быстрый-старт-python)
    - `requirements.py` - Список зависимостей для `python`
  - `auto_install.py` - Установка программы. Скопирывать код из `/builder/template_auto_install.py`
...
1. Шаг раз - пользователь исполняет `auto_install.py`:
   ...
   2. Установить зависимости из файла `server/requirements.txt`, в виртуальное окружение.
...
- Возможность в логе менять адрес для подкючения к серверу
- Сдлеать отправку через бродкаст для подпищиков
- Продумать обновление проагрммы
