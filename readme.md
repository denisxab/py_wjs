...
- По умолчанию будет создан пользователь по имени `app`, в этом пользователи храниться глобальные настройки проекта. Ключи, которые начинаются на `_`(нижнее подчеркивание) зарезервированы для ядра программы, и вы не можете их изменить через функцию `cache_add_key`, вы можете только их прочитать `cache_read_key`.
