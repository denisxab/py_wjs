...
`Pywjs` - альтернативный фреймворк для создания WEB и десктоп приложений. Он не имеет авторских прав, и свободен(бесплатен) для использования в личных, и коммерческих целях.

У меня стаяла задача: Найти готовое решения для создания красивых и удобных GUI программ на `Python`, но не нашел того что меня удовлетворит. Я знаю про существование `PyQt`,`Tkinter`,`Kivy`, я их изучал, и писал кое-какие проекты, знаю что у них есть свои ниши, и не пытаюсь их занять(сдвинуть). Если вы профессионал в `PyQt`,`Tkinter`,`Kivy`, и вам не нужно ни какие другие фреймворки, то закрывайте эту статью. Мне бы хотелось чтобы аудитория, которая знакомиться с этим проектом, была заинтересована в альтернативном взгляде, на создание десктопных программ, и оценила такой подход - как авантюру, которая может перерасти во что-то большее.

> P.s. Я бы хотел чтобы `Python` активно использовали для создания коммерческих десктоп приложения, чтобы было больше таких вакансий, чтобы `Python` программистам которые хотят создавать десктоп приложения, зарабатывали на любимом деле, и не мучились бы со всякими `Delphi`/`Java` потому что только такие, комические вакансии с **низким уровнем входа**, есть на рынке труда. Ричард Мэттью Столлман молодец, что агитирует за свободное ПО(не коммерческое), но быть доставщиком пиццы (на одном из интервью, он так советует программистам зарабатывать на жизнь) у мня плохо получается. Я не имею нечего против свободного ПО, я за свободное ПО, но когда свободные ПО запрещают(или делают платным) для использовать в коммерческих целях, это не свободное ПО -> это проприетарное ПО. Какая вредность от того, что твое ПО используют в коммерческих целях, что кто то обогащаются с помощью него ? В конечном итоге, это же помогает развитию бизнеса, что в следствие помогает развитию общества. Если свободное ПО нельзя использовать в коммерческих целях, то тогда будут использовать проприетарное ПО в коммерческих целях... как говориться в "Притче о двух волка"- "Всегда побеждает тот волк, которого ты кормишь". **Поэтому `Pywjs` свободен(бесплатен) для коммерческого(и не коммерческого) использования**, у него даже нет лицензии и авторского права, поэтому если кото захочет украсть проект - я будут рад форку :)

Идея `Pywjs`:

- Все необходимое должно быть из коробки.
- Все GUI должно быть в браузере. Такой подход позволяет использовать все готовые `WEB` технологии, например сейчас есть поддержка `Vue.js`([Как пользоваться в Vue.js](#как-пользоваться-во-vuejs)). И еще, нам не нужно забоится о кроссплатформенность, потому что эта задача на плечах браузера.
- `Python` выступает в качестве сервер. Обмен данных с браузером происходит по протоколу `WebSocket`, с дополнительными фичами:
  - Используются готовые и стандартизированные `JSON` схемы([JSON Схема](#json-схемы)), похожие на `json-rpc`.
