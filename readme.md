...
В обычном понимание, установка программа на `PywJs` не нужна. Что `html` файл, сразу готов к запуск, что `python`(при наличии зависемых модулей) сразу готов запускыу. Но всеже, для пользоватлей которые, не знаю кто такой `pip`, и как открывать консоль, нуждаются в некоторой автоматизация.
