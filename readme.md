...
- По умолчанию будет создан пользователь по имени `app`, в этом пользователи храниться глобальные настройки проекта. Ключи, которые начинаются на `_`(нижнее подчеркивание) зарезервированы для ядра программы, и вы не можете их изменить через функцию `cache_add_key`, вы можете только их прочитать через функцию `cache_read_key`. [-> Структура пользователя `app`](#структура-пользователя-app)

#### Структура пользователя `app`

TODO: VVVV

**Памятка** для пользователя `app`, в нем нужно хранить ключи которые редко изменяться, и часто используются, или если этот ключ больше некуда(нелогично) сохранить. Эти ключи должны быть полезны для браузера, та как на сервере и так можно получить эти данные.  

> Сокращение ПАПК_ЗП=(Проверяется актуальность при каждом запуске программы, через `auto_run.py`)

1. `_platform` - Информация о платформе на котором запущен сервер
   - `type_os` - Тип операционной системы `windows`,`unix`,`macos`. (`ПАПК_ЗП`)
3. `_auto_update` - Информация для авто обновления программы
   - `git_url` - Ссылка на `git` репозиторий, для синхронизации проекта (`ПАПК_ЗП`)
   - `version_from_pyproject` - Версия из файла `pyproject.toml` (`ПАПК_ЗП`)
4. `_path` - Хранения основных путей программы.
   - `app` - Абсолютный путь к проекту. (`ПАПК_ЗП`)
   - `vnev_python` - Относительный путь(по отношению к `app`) к интерпретатору в виртуальное окружение. Путь сохраняется(обновляется) при создании виртуального окружения через `auto_install.py`
...
- `ИмяПрограммы`
  ...
  - `server` - Для бека
    ...
    - `requirements.py` - Список зависимостей для `python`
    - ...
  - `auto_install.py` - Установка программы. Создать это файл, и скопирывать код из `https://github.com/denisxab/py_wjs/blob/main/builder/auto_install.py`
  - ...

#### Автообновление `pywjs` программы

Пойдем "Быстрее, выше, сильнее" и дадим пользоватлеям, которые так и не познакомились с `pip`-ом и `git`-ом, возможность автоматиечского обновления `pywjs` программы.

**Подрозумивается что вы строго соблюдаете [-> Требуемый шаблон для программы](#требуемый-шаблон-для-программы)**

Две вещи которые происходят в авто обновление программы:

- Синхронизация файлов и директорий через Git. (-)
- Синхронизация верссий модулей в вирутальном окружение `python`, а также скачивание новых модулкй, и удаление стархы. Для этого используется библиотека `poetry`, и в частности файл `pyproject.toml`. (+)
...
- Сделать метод для чтенени файлов
